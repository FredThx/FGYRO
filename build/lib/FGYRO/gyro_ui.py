...
class gyro_ui(object):
	...
	'''
	def __init__(self, bdd, images_folder):
		'''Initialisation
	...
	def init_graphes(self):
		...
		'''
		self.xyz=self.fig.add_subplot(221)
		...
		self.GxGyGz=self.fig.add_subplot(223, sharex = self.xyz)
		...
		self.AxAyAz=self.fig.add_subplot(224, sharex = self.xyz)
		...
		self.image_plot = self.fig.add_subplot(222)
		
		self.fig.canvas.mpl_connect('button_press_event', self.on_click)
		self.bt_lecture = Button(plt.axes([0.9, 0.9, 0.05, 0.03]),'lecture')
		self.bt_lecture.on_clicked(self.on_bt_lecture_click)
		
		self.bt_lecture10 = Button(plt.axes([0.9, 0.8, 0.05, 0.03]),'lecturex10')
